...
def calculate_key_indicators(railway):
    ...
    key_indicators_begin = {
        'Выручка от грузов (млн. руб)': (data['gryzovye_vagoni'] / data['gryz_poesda'] * 120 * data[
            'tonn_per_kilometer']) * uniform(1, 1.05),
        'Себестоимость (коп./10 прив.тонно-км)': (185 * data['gryz_poesda'] + 173 * data['pass_poesda'] * 0.25 + 3840 * data[
            'pass_poesda'] * 0.75) * uniform(1, 1.05),
        'Финансы по прочим видам деятельности (млн. руб)': data['financi_po_prochim'] * 1.02 * (10 ** 6),
        'Дебиторская задолженность (млн. руб)': uniform(0, 100),
        'Кредиторкая задолженность (млн. руб.)': uniform(0, 100),
        'Погрузка грузов (тысячи тонн)': data['gryzovye_vagoni'] + 120,
        'Соблюдение расписания пассажирских поездов (проценты)': (uniform(0.95, 1) + data['sostoyanie_puti'] + data['stat']) / 3,
        'Соблюдение расписания пригородных поездов (проценты)': (uniform(0.95, 1) + data['sostoyanie_puti'] + data['stat']) / 3,
        'Доля грузовых отправок (проценты)': (uniform(0.95, 1) + data['sostoyanie_puti'] + data['stat']) / 3,
        'Средняя скорость пассажирского поезда (км/ч)': ((uniform(0.95, 1) + data['sostoyanie_puti'] + data[
            'stat']) / 3) * int((50 + 35) / 2) / 100,
        'Средняя производительность локомотива (тыс. ткм брутто)': data['gryzovye_vagoni'] / data[
            'gryz_poesda'] * 143 / 365,
        'Время задержки (поездо-час/ млн.поездо-км)': 1 - (
                data['sostoyanie_puti'] + data['sostoyanie_gryz'] + data['sostoyanie_pass'] + data['sostoyanie_loko'] +
                data['stat']) / 5,
        'Безопасность (ед./млн.поездо-км)': 1 - (
                data['sostoyanie_puti'] + data['sostoyanie_gryz'] + data['sostoyanie_pass'] + data['sostoyanie_loko'] +
                data['stat']) / 5,
        'Укомплектованность штата (проценты)': data['stat'],
    }
    ...
    weight_coefficients = {
        'Выручка от грузов (млн. руб)': 0.1,
        'Себестоимость (коп./10 прив.тонно-км)': 0.1,
        'Финансы по прочим видам деятельности (млн. руб)': 0.05,
        'Дебиторская задолженность (млн. руб)': 0.025,
        'Кредиторкая задолженность (млн. руб.)': 0.025,
        'Погрузка грузов (тысячи тонн)': 0.1,
        'Соблюдение расписания пассажирских поездов (проценты)': 0.05,
        'Соблюдение расписания пригородных поездов (проценты)': 0.05,
        'Доля грузовых отправок (проценты)': 0.05,
        'Средняя скорость пассажирского поезда (км/ч)': 0.025,
        'Средняя производительность локомотива (тыс. ткм брутто)': 0.05,
        'Время задержки (поездо-час/ млн.поездо-км)': 0.05,
        'Безопасность (ед./млн.поездо-км)': 0.25,
        'Укомплектованность штата (проценты)': 0.025
    }
    difference = {}
    ...
    for key, value in difference.items():
        ...
        if key == 'Выручка от грузов (млн. руб)':
            weight = 0.1 * uniform(1, 1.10) if value >= 1 else -0.1
        elif key == 'Себестоимость (коп./10 прив.тонно-км)':
            weight = 0.1 * uniform(1, 1.10) if value <= 1 else -0.1
        elif key == 'Финансы по прочим видам деятельности (млн. руб)':
            weight = 0.05 * uniform(1, 1.10) if value >= 1 else -0.05
        elif key == 'Дебиторская задолженность (млн. руб)':
            weight = 0.05 * uniform(1, 1.10) if value <= 1 else -0.05
        elif key == 'Кредиторкая задолженность (млн. руб.)':
            weight = 0.025 * uniform(1, 1.10) if value <= 1 else -0.025
        elif key == 'Погрузка грузов (тысячи тонн)':
            weight = 0.1 * uniform(1, 1.10) if value >= 1 else -0.1
        elif key == 'Соблюдение расписания пассажирских поездов (проценты)':
            weight = 0.05 * uniform(1, 1.10) if value >= 1 else -0.05
        elif key == 'Соблюдение расписания пригородных поездов (проценты)':
            weight = 0.05 * uniform(1, 1.10) if value >= 1 else -0.05
        elif key == 'Доля грузовых отправок (проценты)':
            weight = 0.05 * uniform(1, 1.10) if value >= 1 else -0.05
        elif key == 'Средняя скорость пассажирского поезда (км/ч)':
            weight = 0.025 * uniform(1, 1.10) if value >= 1 else -0.025
        elif key == 'Средняя производительность локомотива/грузового поезда':
            weight = 0.05 * uniform(1, 1.10) if value >= 1 else -0.05
        elif key == 'Время задержки (поездо-час/ млн.поездо-км)':
            weight = 0.05 * uniform(1, 1.10) if value <= 1 else -0.05
        elif key == 'Безопасность (ед./млн.поездо-км)':
            weight = 0.25 * uniform(1, 1.10) if value <= 1 else -0.25
        elif key == 'Укомплектованность штата (проценты)':
            weight = 0.025 * uniform(1, 1.10) if value >= 1 else -0.025
